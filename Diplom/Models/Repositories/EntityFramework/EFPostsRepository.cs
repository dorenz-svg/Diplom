    ...
    public class EFPostsRepository : IPostsRepository
    {
        ...
        public async Task<IEnumerable<PostsResponse>> Get(string id, int count)
        {
            return await Task.FromResult(context.Posts.Include(x => x.Photos)
                ...
                .Skip(20 * count)
                .Take(20)
                .Select(x => new PostsResponse { Id = x.Id, Text = x.Text, Time = x.Time, Likes = context.Likes.Count(c => c.PostId == x.Id), Path = x.Photos.Select(c => configuration.GetConnectionString("ApplicationUrl") + c.Path) }));
        }
